...
def handle_video_set(data):
    ...
    socketio.emit('video_set', {'round_id': round_id, 'video_id': video_id or ''}, room=code)
...
def handle_video_control(data):
    ...
    db.rounds.update_one({'_id': round_id}, {'$set': {
        'video_state': {'playing': action != 'pause', 'position': position, 'position_at_ms': server_ts}
    }})
